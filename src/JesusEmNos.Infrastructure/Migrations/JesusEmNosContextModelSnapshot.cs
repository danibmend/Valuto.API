// <auto-generated />
using System;
using JesusEmNos.Infrastructure.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace JesusEmNos.Infrastructure.Migrations
{
    [DbContext(typeof(JesusEmNosContext))]
    partial class JesusEmNosContextModelSnapshot : ModelSnapshot
    {
        protected override void BuildModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasDefaultSchema("JESUS_EM_NOS_DEV")
                .HasAnnotation("ProductVersion", "9.0.4")
                .HasAnnotation("Relational:MaxIdentifierLength", 60);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Endereco", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Bairro")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("BAIRRO");

                    b.Property<string>("Cep")
                        .IsRequired()
                        .HasMaxLength(8)
                        .HasColumnType("character varying(8)")
                        .HasColumnName("CEP");

                    b.Property<string>("Complemento")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("COMPLEMENTO");

                    b.Property<string>("Logradouro")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("LOGRADOURO");

                    b.Property<long>("MunicipioId")
                        .HasColumnType("bigint")
                        .HasColumnName("MUNICIPIO_ID");

                    b.Property<string>("Numero")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("NUMERO");

                    b.Property<long>("TipoEnderecoId")
                        .HasColumnType("bigint")
                        .HasColumnName("TIPO_ENDERECO_ID");

                    b.HasKey("Id")
                        .HasName("PK_ENDERECO");

                    b.HasIndex("MunicipioId")
                        .HasDatabaseName("IX_ENDERECO_MUNICIPIO_ID");

                    b.HasIndex("TipoEnderecoId")
                        .HasDatabaseName("IX_ENDERECO_TIPO_ENDERECO_ID");

                    b.ToTable("ENDERECO", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Estado", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("NOME");

                    b.Property<string>("Sigla")
                        .IsRequired()
                        .HasMaxLength(2)
                        .HasColumnType("character varying(2)")
                        .HasColumnName("SIGLA");

                    b.HasKey("Id")
                        .HasName("PK_ESTADO");

                    b.HasIndex("Nome")
                        .IsUnique()
                        .HasDatabaseName("IX_ESTADO_NOME");

                    b.HasIndex("Sigla")
                        .IsUnique()
                        .HasDatabaseName("IX_ESTADO_SIGLA");

                    b.ToTable("ESTADO", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Indicador", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Descricao")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("DESCRICAO");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("NOME");

                    b.Property<string>("Sigla")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasColumnName("SIGLA");

                    b.Property<long>("TipoIndicadorId")
                        .HasColumnType("bigint")
                        .HasColumnName("TIPO_INDICADOR_ID");

                    b.HasKey("Id")
                        .HasName("PK_INDICADOR");

                    b.HasIndex("Nome")
                        .IsUnique()
                        .HasDatabaseName("IX_INDICADOR_NOME");

                    b.HasIndex("Sigla")
                        .IsUnique()
                        .HasDatabaseName("IX_INDICADOR_SIGLA");

                    b.HasIndex("TipoIndicadorId")
                        .HasDatabaseName("IX_INDICADOR_TIPO_INDICADOR_ID");

                    b.ToTable("INDICADOR", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Municipio", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("CodIbge7")
                        .IsRequired()
                        .HasMaxLength(7)
                        .HasColumnType("character varying(7)")
                        .HasColumnName("COD_IBGE7");

                    b.Property<long>("EstadoId")
                        .HasColumnType("bigint")
                        .HasColumnName("ESTADO_ID");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("NOME");

                    b.HasKey("Id")
                        .HasName("PK_MUNICIPIO");

                    b.HasIndex("CodIbge7")
                        .IsUnique()
                        .HasDatabaseName("IX_MUNICIPIO_COD_IBGE7");

                    b.HasIndex("EstadoId")
                        .HasDatabaseName("IX_MUNICIPIO_ESTADO_ID");

                    b.ToTable("MUNICIPIO", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Parceiro", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Cnpj")
                        .IsRequired()
                        .HasMaxLength(14)
                        .HasColumnType("character varying(14)")
                        .HasColumnName("CNPJ");

                    b.Property<string>("CpfResponsavel")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("character varying(11)")
                        .HasColumnName("CPF_RESPONSAVEL");

                    b.Property<long>("EnderecoId")
                        .HasColumnType("bigint")
                        .HasColumnName("ENDERECO_ID");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("NOME");

                    b.Property<string>("NomeResponsavel")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("NOME_RESPONSAVEL");

                    b.Property<long>("TipoParceiroId")
                        .HasColumnType("bigint")
                        .HasColumnName("TIPO_PARCEIRO_ID");

                    b.Property<string>("UrlFoto")
                        .HasMaxLength(250)
                        .HasColumnType("character varying(250)")
                        .HasColumnName("URL_FOTO");

                    b.HasKey("Id")
                        .HasName("PK_PARCEIRO");

                    b.HasIndex("Cnpj")
                        .IsUnique()
                        .HasDatabaseName("IX_PARCEIRO_CNPJ");

                    b.HasIndex("EnderecoId")
                        .IsUnique()
                        .HasDatabaseName("IX_PARCEIRO_ENDERECO_ID");

                    b.HasIndex("TipoParceiroId")
                        .HasDatabaseName("IX_PARCEIRO_TIPO_PARCEIRO_ID");

                    b.ToTable("PARCEIRO", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.ParceiroContato", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("ParceiroId")
                        .HasColumnType("bigint")
                        .HasColumnName("PARCEIRO_ID");

                    b.HasKey("Id")
                        .HasName("PK_PARCEIRO_CONTATO");

                    b.HasIndex("ParceiroId")
                        .HasDatabaseName("IX_PARCEIRO_CONTATO_PARCEIRO_ID");

                    b.ToTable("PARCEIRO_CONTATO", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.SolicitacaoParceiro", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Bairro")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("BAIRRO");

                    b.Property<string>("Cep")
                        .IsRequired()
                        .HasMaxLength(8)
                        .HasColumnType("character varying(8)")
                        .HasColumnName("CEP");

                    b.Property<string>("Cnpj")
                        .IsRequired()
                        .HasMaxLength(14)
                        .HasColumnType("character varying(14)")
                        .HasColumnName("CNPJ");

                    b.Property<string>("Complemento")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("COMPLEMENTO");

                    b.Property<string>("CpfResponsavel")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("character varying(11)")
                        .HasColumnName("CPF_RESPONSAVEL");

                    b.Property<DateTime?>("DataResolucao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("DATA_RESOLUCAO");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("EMAIL");

                    b.Property<string>("EmailResponsavel")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("EMAIL_RESPONSAVEL");

                    b.Property<string>("Logradouro")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("LOGRADOURO");

                    b.Property<long>("MunicipioId")
                        .HasColumnType("bigint")
                        .HasColumnName("MUNICIPIO_ID");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("NOME");

                    b.Property<string>("NomeResponsavel")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("NOME_RESPONSAVEL");

                    b.Property<string>("Numero")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("NUMERO");

                    b.Property<long>("SituacaoId")
                        .HasColumnType("bigint")
                        .HasColumnName("SITUACAO_ID");

                    b.Property<string>("Telefone")
                        .IsRequired()
                        .HasMaxLength(13)
                        .HasColumnType("character varying(13)")
                        .HasColumnName("TELEFONE");

                    b.Property<string>("TelefoneResponsavel")
                        .IsRequired()
                        .HasMaxLength(13)
                        .HasColumnType("character varying(13)")
                        .HasColumnName("TELEFONE_RESPONSAVEL");

                    b.Property<long>("TipoEnderecoId")
                        .HasColumnType("bigint")
                        .HasColumnName("TIPO_ENDERECO_ID");

                    b.Property<long>("TipoParceiroId")
                        .HasColumnType("bigint")
                        .HasColumnName("TIPO_PARCEIRO_ID");

                    b.Property<long>("TipoSolicitacaoId")
                        .HasColumnType("bigint")
                        .HasColumnName("TIPO_SOLICITACAO_ID");

                    b.Property<string>("UrlFoto")
                        .HasMaxLength(250)
                        .HasColumnType("character varying(250)")
                        .HasColumnName("URL_FOTO");

                    b.HasKey("Id")
                        .HasName("PK_SOLICITACAO_PARCEIRO");

                    b.HasIndex("MunicipioId")
                        .HasDatabaseName("IX_SOLICITACAO_PARCEIRO_MUNICIPIO_ID");

                    b.HasIndex("SituacaoId")
                        .HasDatabaseName("IX_SOLICITACAO_PARCEIRO_SITUACAO_ID");

                    b.HasIndex("TipoEnderecoId")
                        .HasDatabaseName("IX_SOLICITACAO_PARCEIRO_TIPO_ENDERECO_ID");

                    b.HasIndex("TipoParceiroId")
                        .HasDatabaseName("IX_SOLICITACAO_PARCEIRO_TIPO_PARCEIRO_ID");

                    b.HasIndex("TipoSolicitacaoId")
                        .HasDatabaseName("IX_SOLICITACAO_PARCEIRO_TIPO_SOLICITACAO_ID");

                    b.ToTable("SOLICITACAO_PARCEIRO", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.SolicitacaoVoluntario", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Bairro")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("BAIRRO");

                    b.Property<string>("Cep")
                        .IsRequired()
                        .HasMaxLength(8)
                        .HasColumnType("character varying(8)")
                        .HasColumnName("CEP");

                    b.Property<string>("Complemento")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("COMPLEMENTO");

                    b.Property<string>("Cpf")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("character varying(11)")
                        .HasColumnName("CPF");

                    b.Property<DateTime>("DataNascimento")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("DATA_NASCIMENTO");

                    b.Property<DateTime?>("DataResolucao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("DATA_RESOLUCAO");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("EMAIL");

                    b.Property<long>("IgrejaId")
                        .HasColumnType("bigint")
                        .HasColumnName("IGREJA_ID");

                    b.Property<string>("Logradouro")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("LOGRADOURO");

                    b.Property<long>("MunicipioId")
                        .HasColumnType("bigint")
                        .HasColumnName("MUNICIPIO_ID");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("NOME");

                    b.Property<string>("Numero")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("NUMERO");

                    b.Property<long>("SexoId")
                        .HasColumnType("bigint")
                        .HasColumnName("SEXO_ID");

                    b.Property<long>("SituacaoId")
                        .HasColumnType("bigint")
                        .HasColumnName("SITUACAO_ID");

                    b.Property<string>("Telefone")
                        .IsRequired()
                        .HasMaxLength(13)
                        .HasColumnType("character varying(13)")
                        .HasColumnName("TELEFONE");

                    b.Property<long>("TipoEnderecoId")
                        .HasColumnType("bigint")
                        .HasColumnName("TIPO_ENDERECO_ID");

                    b.Property<long>("TipoSolicitacaoId")
                        .HasColumnType("bigint")
                        .HasColumnName("TIPO_SOLICITACAO_ID");

                    b.Property<string>("UrlFoto")
                        .HasMaxLength(250)
                        .HasColumnType("character varying(250)")
                        .HasColumnName("URL_FOTO");

                    b.HasKey("Id")
                        .HasName("PK_SOLICITACAO_VOLUNTARIO");

                    b.HasIndex("IgrejaId")
                        .HasDatabaseName("IX_SOLICITACAO_VOLUNTARIO_IGREJA_ID");

                    b.HasIndex("MunicipioId")
                        .HasDatabaseName("IX_SOLICITACAO_VOLUNTARIO_MUNICIPIO_ID");

                    b.HasIndex("SexoId")
                        .HasDatabaseName("IX_SOLICITACAO_VOLUNTARIO_SEXO_ID");

                    b.HasIndex("SituacaoId")
                        .HasDatabaseName("IX_SOLICITACAO_VOLUNTARIO_SITUACAO_ID");

                    b.HasIndex("TipoEnderecoId")
                        .HasDatabaseName("IX_SOLICITACAO_VOLUNTARIO_TIPO_ENDERECO_ID");

                    b.HasIndex("TipoSolicitacaoId")
                        .HasDatabaseName("IX_SOLICITACAO_VOLUNTARIO_TIPO_SOLICITACAO_ID");

                    b.ToTable("SOLICITACAO_VOLUNTARIO", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.TipoIndicador", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Descricao")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("DESCRICAO");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("NOME");

                    b.Property<string>("Sigla")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasColumnName("SIGLA");

                    b.HasKey("Id")
                        .HasName("PK_TIPO_INDICADOR");

                    b.HasIndex("Nome")
                        .IsUnique()
                        .HasDatabaseName("IX_TIPO_INDICADOR_NOME");

                    b.HasIndex("Sigla")
                        .IsUnique()
                        .HasDatabaseName("IX_TIPO_INDICADOR_SIGLA");

                    b.ToTable("TIPO_INDICADOR", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Voluntario", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Cpf")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("character varying(11)")
                        .HasColumnName("CPF");

                    b.Property<DateTime>("DataNascimento")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("DATA_NASCIMENTO");

                    b.Property<long>("EnderecoId")
                        .HasColumnType("bigint")
                        .HasColumnName("ENDERECO_ID");

                    b.Property<long>("IgrejaId")
                        .HasColumnType("bigint")
                        .HasColumnName("IGREJA_ID");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("NOME");

                    b.Property<long>("SexoId")
                        .HasColumnType("bigint")
                        .HasColumnName("SEXO_ID");

                    b.Property<string>("UrlFoto")
                        .HasMaxLength(250)
                        .HasColumnType("character varying(250)")
                        .HasColumnName("URL_FOTO");

                    b.HasKey("Id")
                        .HasName("PK_VOLUNTARIO");

                    b.HasIndex("Cpf")
                        .IsUnique()
                        .HasDatabaseName("IX_VOLUNTARIO_CPF");

                    b.HasIndex("EnderecoId")
                        .IsUnique()
                        .HasDatabaseName("IX_VOLUNTARIO_ENDERECO_ID");

                    b.HasIndex("IgrejaId")
                        .HasDatabaseName("IX_VOLUNTARIO_IGREJA_ID");

                    b.HasIndex("SexoId")
                        .HasDatabaseName("IX_VOLUNTARIO_SEXO_ID");

                    b.ToTable("VOLUNTARIO", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.VoluntarioContato", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("ID");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("VoluntarioId")
                        .HasColumnType("bigint")
                        .HasColumnName("VOLUNTARIO_ID");

                    b.HasKey("Id")
                        .HasName("PK_VOLUNTARIO_CONTATO");

                    b.HasIndex("VoluntarioId")
                        .HasDatabaseName("IX_VOLUNTARIO_CONTATO_VOLUNTARIO_ID");

                    b.ToTable("VOLUNTARIO_CONTATO", "JESUS_EM_NOS_DEV");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Endereco", b =>
                {
                    b.HasOne("JesusEmNos.Domain.Entities.Municipio", "Municipio")
                        .WithMany()
                        .HasForeignKey("MunicipioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_ENDERECO_Municipio_MUNICIPIO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "TipoEndereco")
                        .WithMany()
                        .HasForeignKey("TipoEnderecoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_ENDERECO_Indicador_TIPO_ENDERECO_ID");

                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("EnderecoId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("EnderecoId");

                            b1.ToTable("ENDERECO", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("EnderecoId");
                        });

                    b.Navigation("Metadados")
                        .IsRequired();

                    b.Navigation("Municipio");

                    b.Navigation("TipoEndereco");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Estado", b =>
                {
                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("EstadoId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("EstadoId");

                            b1.ToTable("ESTADO", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("EstadoId");
                        });

                    b.Navigation("Metadados")
                        .IsRequired();
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Indicador", b =>
                {
                    b.HasOne("JesusEmNos.Domain.Entities.TipoIndicador", "TipoIndicador")
                        .WithMany()
                        .HasForeignKey("TipoIndicadorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_INDICADOR_TipoIndicador_TIPO_INDICADOR_ID");

                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("IndicadorId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("IndicadorId");

                            b1.ToTable("INDICADOR", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("IndicadorId");
                        });

                    b.Navigation("Metadados")
                        .IsRequired();

                    b.Navigation("TipoIndicador");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Municipio", b =>
                {
                    b.HasOne("JesusEmNos.Domain.Entities.Estado", "Estado")
                        .WithMany("Municipios")
                        .HasForeignKey("EstadoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_MUNICIPIO_ESTADO_ESTADO_ID");

                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("MunicipioId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("MunicipioId");

                            b1.ToTable("MUNICIPIO", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("MunicipioId");
                        });

                    b.Navigation("Estado");

                    b.Navigation("Metadados")
                        .IsRequired();
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Parceiro", b =>
                {
                    b.HasOne("JesusEmNos.Domain.Entities.Endereco", "Endereco")
                        .WithOne()
                        .HasForeignKey("JesusEmNos.Domain.Entities.Parceiro", "EnderecoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_PARCEIRO_ENDERECO_ENDERECO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "TipoParceiro")
                        .WithMany()
                        .HasForeignKey("TipoParceiroId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_PARCEIRO_INDICADOR_TIPO_PARCEIRO_ID");

                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("ParceiroId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("ParceiroId");

                            b1.ToTable("PARCEIRO", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("ParceiroId");
                        });

                    b.Navigation("Endereco");

                    b.Navigation("Metadados")
                        .IsRequired();

                    b.Navigation("TipoParceiro");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.ParceiroContato", b =>
                {
                    b.HasOne("JesusEmNos.Domain.Entities.Parceiro", "Parceiro")
                        .WithMany("Contatos")
                        .HasForeignKey("ParceiroId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_PARCEIRO_CONTATO_PARCEIRO_PARCEIRO_ID");

                    b.OwnsOne("JesusEmNos.Domain.ValueObjects.Contato", "Contato", b1 =>
                        {
                            b1.Property<long>("ParceiroContatoId")
                                .HasColumnType("bigint");

                            b1.Property<long>("ClassificacaoId")
                                .HasColumnType("bigint");

                            b1.Property<long>("TipoContatoId")
                                .HasColumnType("bigint");

                            b1.Property<string>("Valor")
                                .IsRequired()
                                .HasMaxLength(100)
                                .HasColumnType("character varying(100)");

                            b1.HasKey("ParceiroContatoId");

                            b1.HasIndex("ClassificacaoId");

                            b1.HasIndex("TipoContatoId");

                            b1.ToTable("PARCEIRO_CONTATO", "JESUS_EM_NOS_DEV");

                            b1.HasOne("JesusEmNos.Domain.Entities.Indicador", "Classificacao")
                                .WithMany()
                                .HasForeignKey("ClassificacaoId")
                                .OnDelete(DeleteBehavior.Restrict)
                                .IsRequired();

                            b1.WithOwner()
                                .HasForeignKey("ParceiroContatoId");

                            b1.HasOne("JesusEmNos.Domain.Entities.Indicador", "TipoContato")
                                .WithMany()
                                .HasForeignKey("TipoContatoId")
                                .OnDelete(DeleteBehavior.Restrict)
                                .IsRequired();

                            b1.Navigation("Classificacao");

                            b1.Navigation("TipoContato");
                        });

                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("ParceiroContatoId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("ParceiroContatoId");

                            b1.ToTable("PARCEIRO_CONTATO", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("ParceiroContatoId");
                        });

                    b.Navigation("Contato")
                        .IsRequired();

                    b.Navigation("Metadados")
                        .IsRequired();

                    b.Navigation("Parceiro");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.SolicitacaoParceiro", b =>
                {
                    b.HasOne("JesusEmNos.Domain.Entities.Municipio", "Municipio")
                        .WithMany()
                        .HasForeignKey("MunicipioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_PARCEIRO_MUNICIPIO_MUNICIPIO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "Situacao")
                        .WithMany()
                        .HasForeignKey("SituacaoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_PARCEIRO_INDICADOR_SITUACAO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "TipoEndereco")
                        .WithMany()
                        .HasForeignKey("TipoEnderecoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_PARCEIRO_INDICADOR_TIPO_ENDERECO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "TipoParceiro")
                        .WithMany()
                        .HasForeignKey("TipoParceiroId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_PARCEIRO_INDICADOR_TIPO_PARCEIRO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "TipoSolicitacao")
                        .WithMany()
                        .HasForeignKey("TipoSolicitacaoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_PARCEIRO_INDICADOR_TIPO_SOLICITACAO_ID");

                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("SolicitacaoParceiroId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("SolicitacaoParceiroId");

                            b1.ToTable("SOLICITACAO_PARCEIRO", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("SolicitacaoParceiroId");
                        });

                    b.Navigation("Metadados")
                        .IsRequired();

                    b.Navigation("Municipio");

                    b.Navigation("Situacao");

                    b.Navigation("TipoEndereco");

                    b.Navigation("TipoParceiro");

                    b.Navigation("TipoSolicitacao");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.SolicitacaoVoluntario", b =>
                {
                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "Igreja")
                        .WithMany()
                        .HasForeignKey("IgrejaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_VOLUNTARIO_INDICADOR_IGREJA_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Municipio", "Municipio")
                        .WithMany()
                        .HasForeignKey("MunicipioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_VOLUNTARIO_MUNICIPIO_MUNICIPIO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "Sexo")
                        .WithMany()
                        .HasForeignKey("SexoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_VOLUNTARIO_INDICADOR_SEXO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "Situacao")
                        .WithMany()
                        .HasForeignKey("SituacaoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_VOLUNTARIO_INDICADOR_SITUACAO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "TipoEndereco")
                        .WithMany()
                        .HasForeignKey("TipoEnderecoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_VOLUNTARIO_INDICADOR_TIPO_ENDERECO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "TipoSolicitacao")
                        .WithMany()
                        .HasForeignKey("TipoSolicitacaoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_SOLICITACAO_VOLUNTARIO_INDICADOR_TIPO_SOLICITACAO_ID");

                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("SolicitacaoVoluntarioId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("SolicitacaoVoluntarioId");

                            b1.ToTable("SOLICITACAO_VOLUNTARIO", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("SolicitacaoVoluntarioId");
                        });

                    b.Navigation("Igreja");

                    b.Navigation("Metadados")
                        .IsRequired();

                    b.Navigation("Municipio");

                    b.Navigation("Sexo");

                    b.Navigation("Situacao");

                    b.Navigation("TipoEndereco");

                    b.Navigation("TipoSolicitacao");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.TipoIndicador", b =>
                {
                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("TipoIndicadorId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("TipoIndicadorId");

                            b1.ToTable("TIPO_INDICADOR", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("TipoIndicadorId");
                        });

                    b.Navigation("Metadados")
                        .IsRequired();
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Voluntario", b =>
                {
                    b.HasOne("JesusEmNos.Domain.Entities.Endereco", "Endereco")
                        .WithOne()
                        .HasForeignKey("JesusEmNos.Domain.Entities.Voluntario", "EnderecoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_VOLUNTARIO_ENDERECO_ENDERECO_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "Igreja")
                        .WithMany()
                        .HasForeignKey("IgrejaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_VOLUNTARIO_INDICADOR_IGREJA_ID");

                    b.HasOne("JesusEmNos.Domain.Entities.Indicador", "Sexo")
                        .WithMany()
                        .HasForeignKey("SexoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("FK_VOLUNTARIO_INDICADOR_SEXO_ID");

                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("VoluntarioId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("VoluntarioId");

                            b1.ToTable("VOLUNTARIO", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("VoluntarioId");
                        });

                    b.Navigation("Endereco");

                    b.Navigation("Igreja");

                    b.Navigation("Metadados")
                        .IsRequired();

                    b.Navigation("Sexo");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.VoluntarioContato", b =>
                {
                    b.HasOne("JesusEmNos.Domain.Entities.Voluntario", "Voluntario")
                        .WithMany("Contatos")
                        .HasForeignKey("VoluntarioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_VOLUNTARIO_CONTATO_VOLUNTARIO_VOLUNTARIO_ID");

                    b.OwnsOne("JesusEmNos.Domain.ValueObjects.Contato", "Contato", b1 =>
                        {
                            b1.Property<long>("VoluntarioContatoId")
                                .HasColumnType("bigint");

                            b1.Property<long>("ClassificacaoId")
                                .HasColumnType("bigint");

                            b1.Property<long>("TipoContatoId")
                                .HasColumnType("bigint");

                            b1.Property<string>("Valor")
                                .IsRequired()
                                .HasMaxLength(100)
                                .HasColumnType("character varying(100)");

                            b1.HasKey("VoluntarioContatoId");

                            b1.HasIndex("ClassificacaoId");

                            b1.HasIndex("TipoContatoId");

                            b1.ToTable("VOLUNTARIO_CONTATO", "JESUS_EM_NOS_DEV");

                            b1.HasOne("JesusEmNos.Domain.Entities.Indicador", "Classificacao")
                                .WithMany()
                                .HasForeignKey("ClassificacaoId")
                                .OnDelete(DeleteBehavior.Restrict)
                                .IsRequired();

                            b1.HasOne("JesusEmNos.Domain.Entities.Indicador", "TipoContato")
                                .WithMany()
                                .HasForeignKey("TipoContatoId")
                                .OnDelete(DeleteBehavior.Restrict)
                                .IsRequired();

                            b1.WithOwner()
                                .HasForeignKey("VoluntarioContatoId");

                            b1.Navigation("Classificacao");

                            b1.Navigation("TipoContato");
                        });

                    b.OwnsOne("JesusEmNos.Domain.Entities.Base.Metadados", "Metadados", b1 =>
                        {
                            b1.Property<long>("VoluntarioContatoId")
                                .HasColumnType("bigint");

                            b1.Property<DateTime>("DataAtualizacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_ATUALIZACAO");

                            b1.Property<DateTime>("DataCriacao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_CRIACAO");

                            b1.Property<DateTime?>("DataRemocao")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("DATA_REMOCAO");

                            b1.Property<long>("UsuarioAtualizacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_ATUALIZACAO_ID");

                            b1.Property<long>("UsuarioCriacaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_CRIACAO_ID");

                            b1.Property<long?>("UsuarioRemocaoId")
                                .HasColumnType("bigint")
                                .HasColumnName("USUARIO_REMOCAO_ID");

                            b1.HasKey("VoluntarioContatoId");

                            b1.ToTable("VOLUNTARIO_CONTATO", "JESUS_EM_NOS_DEV");

                            b1.WithOwner()
                                .HasForeignKey("VoluntarioContatoId");
                        });

                    b.Navigation("Contato")
                        .IsRequired();

                    b.Navigation("Metadados")
                        .IsRequired();

                    b.Navigation("Voluntario");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Estado", b =>
                {
                    b.Navigation("Municipios");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Parceiro", b =>
                {
                    b.Navigation("Contatos");
                });

            modelBuilder.Entity("JesusEmNos.Domain.Entities.Voluntario", b =>
                {
                    b.Navigation("Contatos");
                });
#pragma warning restore 612, 618
        }
    }
}
